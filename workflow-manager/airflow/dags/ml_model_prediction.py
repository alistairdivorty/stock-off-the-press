...
def spark_job_driver(
    ...
    db_connection_uri = urlunsplit(
        (
            "mongodb",
            f"{os.environ['DOCDB_MASTER_USERNAME']}:{os.environ['DOCDB_MASTER_USER_PASSWORD']}@{docdb_stack_output_value('DocDBEndpoint')}:{docdb_stack_output_value('DocDBPort')}",
            os.environ["DOCDB_DATABASE_NAME"],
            urlencode(
                dict(
                    replicaSet="rs0",
